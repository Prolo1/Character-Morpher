...
namespace Character_Morpher
{
    ...
    class CharaMorpher_GUI : ProloGUIBehaviour<CharaMorpher_GUI>
    {
        #region Classes
        ...
        internal static CharaMorpher_Controller studioMorphCtrl = null;
        ...
        static void ImmediateGUI()
        {
            ...
            GUI.DrawTexture(winRec = GUI.Window(CharaMorpher_Core.GUID.GetHashCode(),
                winRec, id =>
                {
                    ...
                    if(cfg.enableTooltips.Value && !tooltip.IsNullOrEmpty())
                    {
                        if(tmpSty == null)
                        {
                            tmpSty = new GUIStyle(GUI.skin.label)
                            {
                                normal = new GUIStyleState
                                {
                                    ...
                                    background = CreateColourTexture(new Color(0, 0, 0, .5f))
                                },
                                wordWrap = true,
        ...
        internal static void Initialize()
        {
            ...
            if(InsideStudio)
            {
                ...
                {
                    var Logger = CharaMorpher_Core.Logger;
                ...
                lastUCMDEvent = (s, o) =>
                {
                    IEnumerator CoUCMD()
                    {
                        yield return studioMorphCtrl?.StartCoroutine(CoUCMDCommon(studioMorphCtrl, lastUCMD));
                    ...
                    if(tmp != null)
                        studioMorphCtrl.StopCoroutine(tmp);
                ...
                customStudioUI.AddListener(() =>
                {
                    ...
                    try
                    {
                        ...
                        //Update checks
                        {
                            ...
                            if(studioMorphCtrl)
                            {
                                if(charEnable != studioMorphCtrl.morphEnable)
                                    studioMorphCtrl.Enable = charEnable;

                                if(charEnableABMX != studioMorphCtrl.morphEnableABMX)
                        ...
                        foreach(var cat in inst.controlCategories[defaultStr])
                            if(cat.dataName.ToLower().Contains("voice"))
                                CreateVoiceSlider(cat.dataName, studioMorphCtrl);
                            else
                                CreateShapeSlider(cat.dataName, studioMorphCtrl);
                        ...
                        if(!init)
                        {
                            ...
                            dropdown = GUILayoutDropdownDrawer(
                                ...
                                listUpdate: (old) =>
                                {
                                    if(last != studioMorphCtrl || !init)
                                        return
                                        (studioMorphCtrl?.controls?.all?.Keys.ToList() ??
                                ...
                                modSelected: (selected) =>
                                {
                                    if(last != studioMorphCtrl)
                                    {
                                        ...
                                        k.Substring(0, k.LastIndexOf(strDiv)) : throw new Exception())
                                        .ToArray() ??
                                        new string[0], studioMorphCtrl?.controls?.currentSet);
                                        last = studioMorphCtrl;
                                    }
                        ...
                        #region Buttons
                        var MorphBackupCtrl = (!studioMorphCtrl?.IsUsingExtMorphData ?? true ? studioMorphCtrl?.ctrls1 :
                                    (studioMorphCtrl?.ctrls2 ?? studioMorphCtrl?.ctrls1));

                        GUILayout.BeginHorizontal();
                        if(GUILayout.Button("Load From Current Slot") && studioMorphCtrl)
                        {
                            studioMorphCtrl.controls.Copy(MorphBackupCtrl);
                            studioMorphCtrl.controls.currentSet = cfg.currentControlSetName.Value;
                        ...
                        {
                            AddNewSetting(ctrl1: studioMorphCtrl, useUserDefault: useDefaultAsDefault);

                            //update dropdown list				 
                            //UpdateGUISelectList();
                            SwitchControlSet((studioMorphCtrl?.controls?.all?.Keys?.ToList() ??
                                        inst?.controlCategories?.Keys.ToList())
                                        .Attempt((k) => k.LastIndexOf(strDiv) >= 0 ?
                                        k.Substring(0, k.LastIndexOf(strDiv)) : throw new Exception())
                                        .ToArray() ??
                        ...
                        if(GUILayout.Button("Remove Current Slot"))
                        {
                            ...
                            var list = (studioMorphCtrl?.controls?.all?.Keys?.ToList() ??
                                        inst?.controlCategories?.Keys.ToList())
                                        .Attempt((k) => k.LastIndexOf(strDiv) >= 0 ?
                                        k.Substring(0, k.LastIndexOf(strDiv)) : throw new Exception())
                                        .ToArray() ??
                            ...
                            RemoveCurrentSetting(studioMorphCtrl?.controls?.currentSet ?? cfg.currentControlSetName.Value, studioMorphCtrl);
        ...
        private static void AddCharaMorpherMenu(RegisterCustomControlsEvent e)
        {
            ...
            e.AddLoadToggle(new MakerLoadToggle("Chara Morph.", initialValue: MorphLoadToggle))
                ...
                }).tooltipMsg("Allows the morph data to be read by mod. If disabled, card will be loaded like normal", Instance, () => cfg.enableTooltips.Value);
            ...
            e.AddLoadToggle(new MakerLoadToggle("Chara Morph.\nEnables", initialValue: MorphCharaSpecificEnablesToggle))
                .OnGUIExists((gui) =>
                {
                    ...
                    MorphCharaSpecificEnablesToggle = gui.Value;
                        ...
                        "If disabled, the \"Chara Enable\" options will not be overridden by card data. " +
                        "If \"Chara Morph.\" is disabled this toggle will also be treated as disabled.", Instance, () => cfg.enableTooltips.Value);

            #endregion

            #region Enables
            e.AddControl(new MakerText("Enablers", category, inst));

            e.AddControl(new MakerToggle(category, "Enable", cfg.enable.Value, inst))
              .OnGUIExists((gui) =>
              {
              ...
              }).tooltipMsg(cfg.enable.Description.Description, Instance, () => cfg.enableTooltips.Value)
            ...
            if(ABMXDependency.IsInTargetVersionRange)
                e.AddControl(new MakerToggle(category, "Enable ABMX", cfg.enableABMX.Value, inst))
                  .OnGUIExists((gui) =>
                  {
                  ...
                  }).tooltipMsg(cfg.enableABMX.Description.Description, Instance, () => cfg.enableTooltips.Value)
                  ?.BindToFunctionController<CharaMorpher_Controller, bool>(
            ...
            e.AddControl(new MakerToggle(category, "Chara. Enable", true, inst))
              .OnGUIExists((gui) =>
              {
              ...
              }).tooltipMsg("Character specific enable button (gets saved with card)", Instance, () => cfg.enableTooltips.Value);

            if(ABMXDependency.IsInTargetVersionRange)
                e.AddControl(new MakerToggle(category, "Chara. Enable ABMX", true, inst))
                  .OnGUIExists((gui) =>
                  {
                      var ctrl = GetCharacterControl().GetComponentInParent<CharaMorpher_Controller>();
                  ...
                  }).tooltipMsg("Character specific enable ABMX button (gets saved with card)", Instance, () => cfg.enableTooltips.Value);


            e.AddControl(new MakerToggle(category, "Save Ext. Data", cfg.saveExtData.Value, inst))
               .OnGUIExists((gui) =>
               {
               ...
               }).tooltipMsg(cfg.saveExtData.Description.Description, Instance, () => cfg.enableTooltips.Value)
               .BindToFunctionController<CharaMorpher_Controller, bool>
               ((ctrl) => cfg.saveExtData.Value,
               (ctrl, val) => { if(val != cfg.saveExtData.Value) cfg.saveExtData.Value = val; });


            e.AddControl(new MakerToggle(category, "Link Overall Sliders to ABMX Overall Sliders", cfg.linkOverallABMXSliders.Value, inst))
              .OnGUIExists((gui) =>
              {
              ...
              }).tooltipMsg(cfg.linkOverallABMXSliders.Description.Description, Instance, () => cfg.enableTooltips.Value)
              .BindToFunctionController<CharaMorpher_Controller, bool>
            ...
            var enableQuadManip = e.AddControl(new MakerToggle(category, "Enable Calculation Types", cfg.enableCalcTypes.Value, inst))
                .OnGUIExists((gui) =>
                {
                ...
                }).tooltipMsg(cfg.enableCalcTypes.Description.Description, Instance, () => cfg.enableTooltips.Value);

            _ = e.AddControl(new MakerToggle(category, "Use Card Morph Data", cfg.preferCardMorphDataMaker.Value, inst))
                        .OnGUIExists((gui) =>
                        {
                            gui?.ValueChanged?.Subscribe((_1) =>
                    {
                        ...
                        }).tooltipMsg(cfg.preferCardMorphDataMaker.Description.Description, Instance, () => cfg.enableTooltips.Value);

            _ = e.AddControl(new MakerToggle(category, "Load Init. Character", cfg.loadInitMorphCharacter.Value, inst))
                .OnGUIExists((gui) =>
                {
                ...
                }).tooltipMsg(cfg.loadInitMorphCharacter.Description.Description, Instance, () => cfg.enableTooltips.Value);

            e.AddControl(new MakerToggle(category, "User Default As Default", cfg.userDefaultAsDefault.Value, inst))
                .OnGUIExists((gui) =>
                {

                    cfg.userDefaultAsDefault.SettingChanged +=
                    userDefaultAsDefaultEvent = (m, n) =>
                    {
                        if(gui?.Value != cfg.userDefaultAsDefault.Value)
                            gui?.SetValue(cfg.userDefaultAsDefault.Value);
                    };
                    gui.ValueChanged.Subscribe((tgl) => cfg.userDefaultAsDefault.Value = tgl);
                    //tooltipMsg(cfg.userDefaultAsDefault.Description.Description, gui);
                }).tooltipMsg(cfg.userDefaultAsDefault.Description.Description, Instance, () => cfg.enableTooltips.Value);

            e.AddControl(new MakerToggle(category, "Enable Tooltips", cfg.enableTooltips.Value, inst))
                .OnGUIExists(gui =>
                {
            ...
            e.AddControl(new MakerButton("Reset To Original Shape", category, inst))
                .OnGUIExists((gui) =>
                {
                ...
                }).tooltipMsg("resets shape of character to how it looked when disabled (will not change morph values)", Instance, () => cfg.enableTooltips.Value);
                ...
                float min = 0, float max = 1)
            {
                ...
                {
                    e.AddControl(new MakerText("", category, inst));//create space
                    ...
                    e.AddControl(new MakerText($"{part} Controls".Trim(), category, inst));
                }
                ...
                //setup slider
                var currSlider = sliders.AddNReturn(e.AddControl(new MorphMakerSlider(category, visualName.Trim(), min, max, (float)cfg.defaults[cfg.currentControlSetName.Value][settingName].Value.data, inst)));
                currSlider.BindToFunctionController<CharaMorpher_Controller, float>(
                        ...
                        (ctrl, val) =>
                        {
                            if(cfg.debug.Value) Logger.LogDebug($"called slider");
                            if(cfg.debug.Value) Logger.LogDebug($" ctrl is:{(ctrl ? "Exists" : "Null")}");
                            if(!ctrl) return;
                            if(cfg.debug.Value) Logger.LogDebug($" ctrl.IsInitLoadFinished is:{ctrl.IsInitLoadFinished}");
                            if(cfg.debug.Value) Logger.LogDebug($" ctrl.IsReloading is:{ctrl.IsReloading}");
                            if(cfg.debug.Value) Logger.LogDebug($" {ctrl.controls.all[ctrl.controls.currentSet][settingName].data} == {(float)Math.Round(val, 2)}");
                            ...
                            if(cfg.debug.Value) Logger.LogDebug($"edited slider");
                ...
                var currMode = modes.AddNReturn(e.AddControl(new MorphMakerDropdown("", ting, category, (int)cfg.defaults[cfg.currentControlSetName.Value][settingName].Value.calcType, inst)));
                ...
                OnInternalSliderValueChange.AddListener(sliderValActions.AddNReturn((_) =>
                {
                    ...
                    CharaMorpher_Controller ctrl = GetAllChaFuncCtrlOfType<CharaMorpher_Controller>()?.FirstOrNull(k => k != null);//first one only
                ...
                //add separator after overall control
                if(Regex.IsMatch(settingName, searchHits[0], RegexOptions.IgnoreCase))
                    e.AddControl(new MakerSeparator(category, inst));//create separator line
            ...
            var sep = e.AddControl(new MakerSeparator(category, inst))
                 .AddToCustomGUILayout(-1, debug: cfg.debug.Value);

            select = e.AddControl(
                new MorphMakerDropdown("Selected Slot", ControlsList, category, 0, inst))
                .AddToCustomGUILayout(-1, debug: cfg.debug.Value)
                .OnGUIExists((gui) =>
                {
                    ...
                    gui.ValueChanged?.Subscribe((val) =>
                    {
                        gui.Value = SwitchControlSet(gui.Options, val);
                        Illusion.Game.Utils.Sound.Play(Illusion.Game.SystemSE.sel);
                    });
                    gui.Value = SwitchControlSet(gui.Options, cfg.currentControlSetName.Value);
                });


            e.AddControl(new MakerButton("Add New Slot", category, inst))
                .AddToCustomGUILayout(-1, newVertLine: true, debug: cfg.debug.Value)
                .OnClick.AddListener(() =>
                   {
                       AddNewSetting();
            ...
            e.AddControl(new MakerButton("Remove Current Slot", category, inst))
                .AddToCustomGUILayout(-1, newVertLine: false, debug: cfg.debug.Value)
                .OnClick.AddListener(() =>
                {
            ...
            e.AddControl(new MakerButton("Save To Slot", category, inst))
                .AddToCustomGUILayout(-1, newVertLine: true, debug: cfg.debug.Value)
                .OnClick.AddListener(() =>
                {
                    foreach(var slider in sliders)
            ...
            e.AddControl(new MakerButton("Load From Slot", category, inst))
               .AddToCustomGUILayout(cfg.makerViewportUISpace.Value, newVertLine: false, debug: cfg.debug.Value)
               .OnGUIExists((gui) => loadDefaultValues = (bool showMessage, bool playSound, bool runUpdate) =>
               {
                   var ctrl = GetAllChaFuncCtrlOfType<CharaMorpher_Controller>().First();
        ...
        private static void ImageControls(RegisterCustomControlsEvent e, BaseUnityPlugin owner)
        {
            ...
            e.AddControl(new MakerText("Morph Target", category, inst));
            ...
            OnNewTargetImage.AddListener(
                (path, png) =>
                {
                    ...
                    inst.StartCoroutine(CoSetTexture(path, png));
                });
        ...
        private static void ButtonDefaults(RegisterCustomControlsEvent e, BaseUnityPlugin owner)
        {
            ...
            //e.AddControl(new MakerText("", category, CharaMorpher_Core.Instance));//create space
            e.AddControl(new MakerText("Easy Morph Buttons", category, inst));
