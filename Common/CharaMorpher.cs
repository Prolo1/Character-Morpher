    ...
    public partial class CharaMorpher_Core : ProloUnityPlugin<CharaMorpher_Core>
    {
        ...
        void Awake()
        {
            ...
            cfg = new MorphConfig
            {
                ...
                new ConfigDescription("Enables quadratic mode where value gets squared (i.e. 1.2 = pow(1.2, 2) = 1.44)", null,
                new ConfigurationManagerAttributes { Order = --index, Category = mainx, })),
                saveExtData = Config.Bind(main, "Save Ext. Data", true,
                new ConfigDescription("Allows the card to save using extended data. " +
                "\n\nIf true, card is saved as seen with Morph Extended data added to the card (card will have originl body values when someone doesn't have the mod), " +
                "\n\nelse the card is saved normally w/o Morph Extended data and saved as seen (must be set before saving obviously)", null,
                new ConfigurationManagerAttributes { Order = --index, Category = mainx, })),
